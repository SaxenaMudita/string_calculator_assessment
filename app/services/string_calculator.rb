class StringCalculator
  ...
  def self.custom_delimiters?(numbers)
    numbers.start_with?("//[")
  end

  def self.single_custom_delimiter?(numbers)
    numbers.start_with?("//")
  end

  def self.parse_multiple_delimiters(numbers)
    delimiters = numbers.scan(/\[(.*?)\]/).flatten
    delimiter_regex = delimiters.map { |d| Regexp.escape(d) }.join("|")
    [ delimiter_regex, numbers.split("\n", 2).last ]
  end

  def self.parse_single_delimiter(numbers)
